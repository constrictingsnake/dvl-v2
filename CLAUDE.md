...
- [x] **6. Define the Firestore schema.** `firestore.rules` at repo root with a strict **deny-all baseline** (`allow read, write: if false`; per-user rules land in Phase 1, hardening in Phase 5). TS types in **`packages/firebase/src/types.ts`** (src layout, not the literal `packages/firebase/types.ts`): `Item`, `ItemHistory`, `User` + `Site`/`ItemStatus` unions, timestamps typed as client `Timestamp` from `firebase/firestore`, `fcmTokens` on `User` (not `Item`). Re-exported from `src/index.ts` (`export type { ... }`). ⚠️ Deny-all blocks client writes, so Phase 0 dev/test writes go through the Admin SDK or seeded emulator data. *Done-when ✅: types compile + are exported from the package; rules file present; typecheck/lint/format clean.*
