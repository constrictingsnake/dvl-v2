...
**Refined minimalism with a single hot-pink accent.** The app's visual identity is themed (loosely) on Diavolo from JoJo's Bizarre Adventure: Italian high-fashion, elegant, restrained — soft surfaces, one vivid pop. The discipline is in *not* adding things; **when in doubt, remove it.** All UI is Tailwind, built directly in the extension.

- **Color — neutral base, one accent, used once per view.** Backgrounds `bg-white` / `bg-neutral-50`. Text hierarchy carries most of the work: `text-neutral-900` (primary) → `text-neutral-700` (body) → `text-neutral-500` (muted/secondary). The hot-pink accent lives in the Tailwind token **`brand` (`#ec4899`)** — apply it to **at most one element per view** (the primary action, a highlight, a focus ring). The moment pink is on two things it stops being an accent. **Pastel pink fails as text on white** (the original `#f9d8e0` looked disabled) — pink is a *fill/accent* color, not body text. If a soft surface is ever needed, add a separate token (e.g. `surface`) rather than overloading `brand`.
- **Type — Bodoni Moda titles, Inter body.** Both self-hosted via Fontsource (`@fontsource-variable/{inter,bodoni-moda}`, bundled — no CDN, CWS-safe). `font-display` → titles/wordmarks ONLY; everything else inherits Inter (the `sans` token is overridden so Inter is the app-wide default — no class needed on `body`). Restrained size ladder (`text-2xl` titles · `text-sm` UI/body · `text-xs` labels); quiet weights (default body, `font-medium` for emphasis — avoid blanket `font-bold`, let Bodoni carry the elegance).
- **Spacing — whitespace is the design.** Pick a rhythm and reuse it: `space-y-2`/`space-y-3` between elements, `p-6` surface padding, `mb-6` between sections. Group with `space-y-*` on the parent rather than margins on each child.
- **Borders & shadows — almost none.** No shadows, no gradients. At most a single hairline (`border-b border-neutral-200`) to separate sections. Corners: pick ONE rule (sharp, or a consistent `rounded-md`) and apply it everywhere — don't mix.
- **Components — one loud thing, everything else quiet.** One primary action per view (the pink accent); secondary actions are ghost/text buttons (`text-neutral-500 hover:text-neutral-900`, no background). Prefer left-aligned text over centered (calmer, more editorial). Note `<button>` defaults to `text-align: center` — add `text-left` if you want labels left-aligned.
...
- [ ] **0. Repo hygiene (ongoing).** Branch `phase-1-…` off `main` (after Phase 0 merges); commit per step; keep `pnpm typecheck/lint/format:check/test` green before each commit. *Done-when: work is on a `phase-1` branch, each step committed.*
- [ ] **1. UI foundation (Tailwind + surfaces).** Install/configure **Tailwind CSS** (+ PostCSS) and **Lucide** icons in `apps/extension`; strip the WXT demo chrome; establish the two UI surfaces — a compact **popup** and a full-page **dashboard** entrypoint (`apps/extension/entrypoints/dashboard/`) opened from the popup. (Sidepanel deferred — full page is simpler for the rich view.) *Stubs Claude generates:* `tailwind.config`, PostCSS wiring, base CSS, `<Popup>` + `<Dashboard>` shells with `// TODO:` layout. *Done-when: both surfaces build and render styled placeholders; Tailwind utility classes apply in the production build; lint/format/typecheck clean.*
- [ ] **2. App state store (Zustand).** Install **Zustand**; scaffold `apps/extension/store/` typed against `AuthUser`, `WithId<Item>[]`, and a UI slice (`status: 'loading'|'ready'|'error'`, `error`, and placeholders for filter/sort/search). Actions/selectors stubbed. *Stubs:* `store/useAppStore.ts` with typed state + `// TODO:` action bodies. *Done-when: store is typed and imported by popup/dashboard; typecheck clean.*
- [ ] **3. Baseline security rules + rules tests.** Replace the deny-all `firestore.rules` with **path-based per-user** rules: `match /users/{uid}/{document=**} { allow read, write: if request.auth.uid == uid; }` (use `request.resource.data`, not `resource.data`, for any create/update predicate). Add **`@firebase/rules-unit-testing`** tests on the Firestore emulator (own-doc R/W allowed; other-uid denied; unauthenticated denied), wired into Vitest + CI. *Stubs:* updated `firestore.rules` with `// TODO:` predicates, `tests/firestore.rules.test.ts` with `// TODO:` cases + emulator harness. *Done-when: rules tests green in CI; own-user R/W allowed, cross-user + unauth denied.*
- [ ] **4. Auth flow + user-doc bootstrap.** Turn the Phase-0 smoke button into a real **signed-out (sign-in screen) ↔ signed-in (dashboard)** flow driven by `observeAuthState` → store. On first sign-in, **transactionally create `users/{uid}`** (`createdAt`, `email`, `displayName`, `itemCount: 0`, `defaultNotify`, `fcmTokens: {}`) — idempotent on repeat sign-in. Wire sign-out. This is the **first client Firestore write** (permitted by step 3's rules). *Stubs:* `ensureUserDoc(user)` with `// TODO:` transaction, an auth-gating route/wrapper component, sign-in/out UI shells. *Done-when: sign-in creates the user doc (visible in the emulator/console), idempotent on repeat; sign-out returns to the sign-in screen; auth persists across reopen.*
- [ ] **5. Firestore real-time listener.** `onSnapshot` on `users/{uid}/items` → store; **subscribe on sign-in, unsubscribe on sign-out**; map Firestore docs → `WithId<Item>` (Timestamp→epoch-ms where the UI needs it). *Stubs:* `lib/items.ts` `subscribeToItems(uid, cb): Unsubscribe` + store wiring, all `// TODO:`. *Done-when: a doc added directly in the emulator appears live in the UI with no reload; sign-out tears the listener down (no leaks).*
- [ ] **6. Item card + dashboard list.** `<ItemCard>` (title, image, **price formatted from minor-units cents + ISO `currency`**, bid count, end time, `status` + `bidStatus` badges) and the dashboard list rendering store items, with **empty** and **loading** states. Built directly in the extension (no `packages/ui` — single consumer). *Stubs:* `components/ItemCard.tsx`, `lib/formatMoney.ts` (`formatMoney(cents, currency)`), `components/ItemList.tsx`, `// TODO:` render bodies. *Done-when: live items render as cards; empty/loading states show; money + time formatting correct.*
- [ ] **7. Manual add (paste URL → `pending` item).** A form that validates a listing URL, **detects `site` from the host** (small `siteFromUrl` util in `packages/adapters`), and **transactionally** writes a `pending` `Item` to `users/{uid}/items` while bumping `itemCount` — enforcing the **advisory** `MAX_ITEMS_PER_USER` cap from `functions/src/limits.ts` (⚠️ client-side/advisory only; real enforcement is the Phase 3 trigger). Data hydration is deferred to the Phase 3 poller (item stays `pending`). Completes the write→listener→card loop with no content scripts. *Stubs:* `components/AddItemForm.tsx`, `adapters/siteFromUrl.ts`, `lib/items.ts` `createPendingItem(uid, url)` with `// TODO:` transaction + validation. *Done-when: pasting a valid URL creates a `pending` card live; `itemCount` increments; invalid URL and over-cap are rejected with a message.*
- [ ] **8. Remove item.** Capture is one-way (no un-save detection), so the dashboard owns removal: **transactionally** delete the item doc and decrement `itemCount`. (Recursive history-subcollection delete is noted for later — no `history` docs exist until Phase 3.) *Stubs:* `lib/items.ts` `removeItem(uid, itemId)` + a card control, `// TODO:`. *Done-when: removing a card deletes the doc and updates the count live.*
- [ ] **9. Dashboard shell polish — filter / sort / search / grouping.** Client-side over the store: title **search**, **sort** (end time / price / date added), **status filter**, and watchlist **`group`** surfacing. Keep it minimal — this is shell polish, not the engine. *Stubs:* store UI-slice actions + `components/DashboardToolbar.tsx`, `// TODO:` handlers + a `selectVisibleItems` selector. *Done-when: search/sort/filter reflow the list; items bucket by `group`.*
